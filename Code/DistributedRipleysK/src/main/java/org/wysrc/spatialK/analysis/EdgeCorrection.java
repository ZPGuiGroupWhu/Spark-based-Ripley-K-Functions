package org.wysrc.spatialK.analysis;

import org.locationtech.jts.algorithm.CGAlgorithms;
import org.locationtech.jts.geom.*;
import org.wysrc.spatialK.geom.SpatiotemporalPoint;
import org.wysrc.spatialK.geom.SpatiotemporalPolygon;
...
public class EdgeCorrection {
    ...
    public static double isotropicCorrection(SpatiotemporalPoint centerPoint, SpatiotemporalPoint neighborPoint,
        ...
        double temporalWeight = temporalIsotropicCorrection(temporalCenterPoint, temporalNeighborPoint, startTime, endTime);
    ...
    public static double temporalIsotropicCorrection(LocalDateTime centerTime, LocalDateTime neighborTime,
        ...
        long interval = Duration.between(centerTime, neighborTime).abs().getSeconds();
        if(interval == 0) {
            return 1.0;
        }
        double temporalWeight;
        long totalLength = Duration.between(startTime, endTime).abs().getSeconds();
        LocalDateTime lowerTime = centerTime.minus(interval, ChronoUnit.SECONDS);
        LocalDateTime higherTime = centerTime.plus(interval, ChronoUnit.SECONDS);
        if(lowerTime.compareTo(startTime) < 0) {
            if(higherTime.compareTo(endTime) < 0) {
                temporalWeight = Duration.between(startTime, higherTime).abs().getSeconds() / (2.0 * interval);
            } else {
                temporalWeight = totalLength / (2.0*interval);
            }
        } else {
            if(higherTime.compareTo(endTime) < 0) {
                temporalWeight = 1.0;
            } else {
                temporalWeight = Duration.between(lowerTime, endTime).abs().getSeconds() / (2.0 * interval);
            }
        }

        temporalWeight = 1 / temporalWeight;
        temporalWeight = Math.min(temporalWeight, weightMaximum);
        temporalWeight = Math.max(weightMinimum, temporalWeight);
...
//        System.out.println(boundary);
        ...
        double temporalWeight = temporalIsotropicCorrection(centerTime, neighborTime, startTime, endTime);
