package org.wysrc.spatiotemporalK.geom;
...
public class SpatiotemporalEnvelope implements Comparable, Serializable {
    ...
    public long getTemporalInterval(TemporalUnit temporalUnit) {
        if(temporalUnit instanceof ChronoUnit) {
            ...
            switch (chronoUnit) {
                case YEARS: {
                    return ChronoUnit.YEARS.between(startTime.toLocalDate(), endTime.toLocalDate());
                }
                case MONTHS: {
                    return ChronoUnit.MONTHS.between(startTime.toLocalDate(), endTime.toLocalDate());
                }
                case DAYS: {
                    return ChronoUnit.DAYS.between(startTime.toLocalDate(), endTime.toLocalDate());
                }
                case HOURS: {
                    return Duration.between(startTime, endTime).abs().toHours();
                }
                case MINUTES: {
                    return Duration.between(startTime, endTime).abs().toMinutes();
                }
                case SECONDS: {
                    return Duration.between(startTime, endTime).abs().getSeconds();
                }
            }
            throw new UnsupportedTemporalTypeException("Unsupported unit: " + temporalUnit);
        }
        return temporalUnit.between(startTime, endTime);
    }
