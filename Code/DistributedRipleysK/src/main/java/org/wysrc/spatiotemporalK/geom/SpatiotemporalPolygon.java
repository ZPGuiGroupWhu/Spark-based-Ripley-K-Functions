    ...
    public double getVolume(TemporalUnit temporalUnit) {
        if(temporalUnit instanceof ChronoUnit) {
            ...
            switch (chronoUnit) {
                ...
                case YEARS: {
                    long temporalInterval = ChronoUnit.YEARS.between(startTime.toLocalDate(), endTime.toLocalDate());
                ...
                case MONTHS: {
                    long temporalInterval = ChronoUnit.MONTHS.between(startTime.toLocalDate(), endTime.toLocalDate());
                    if(temporalInterval == 0) {
                        return this.spatialGeometry.getArea();
                    } else {
                        return temporalInterval * this.spatialGeometry.getArea();
                ...
                case DAYS: {
                    long temporalInterval = ChronoUnit.DAYS.between(startTime.toLocalDate(), endTime.toLocalDate());
                    if(temporalInterval == 0) {
                        return this.spatialGeometry.getArea();
                    } else {
                        return temporalInterval * this.spatialGeometry.getArea();
