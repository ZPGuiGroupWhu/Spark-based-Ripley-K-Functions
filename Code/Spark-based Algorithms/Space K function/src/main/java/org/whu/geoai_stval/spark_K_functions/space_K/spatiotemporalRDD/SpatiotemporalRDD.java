package org.whu.geoai_stval.spark_K_functions.space_K.spatiotemporalRDD;
...
import org.whu.geoai_stval.spark_K_functions.space_K.index.IndexBuilder;
...
public class SpatiotemporalRDD<T extends SpatiotemporalGeometry> implements Serializable {
    ...
    public void buildSpatiotemporalPartitioner(PartitionerType partitionerType, int numPartitions) throws Exception {
        ...
        if (all) {
            ...
            if (this.totalCount < TOTAL_COUNT_THRESHOLD) {
                ...
                if (this.sampleCount < 0 || this.sampleCount > this.totalCount) {
                    //this.sampleCount = Math.max(numPartitions * 2, Math.min((int) this.totalCount * 2 / 100, Integer.MAX_VALUE));
        ...
        switch (partitionerType) {
            ...
            case KDBTree: {
                /*
                   limit is  maxChildPerNode,there may have a bug when it is lesser than 2.
                 * */
                int limit = samples.size() / numPartitions;
                limit = limit < 2 ? 2 : limit;
                KDBTree kdbTree = new KDBTree(limit, limit, envelope);
...
//        JavaPairRDD<Integer, T> tmp = this.rawRDD.flatMapToPair(new PairFlatMapFunction<T, Integer, T>() {
//            @Override
...
//            }
//        });
//        JavaRDD<T> result = tmp.partitionBy(partitioner).mapPartitions(
...
//        return result;
    ...
    public void generateCuboidPartitioner4RawRDD() {
        List<SpatiotemporalEnvelope> cuboids = this.rawRDD.map(g -> g.getEnvelopeInternal()).mapPartitionsWithIndex(
            ...
            , true).collect();
//        List<Tuple2<String, Long>> collect = this.rawRDD.mapPartitionsWithIndex(new Function2<Integer, Iterator<T>, Iterator<Tuple2<String, Long>>>() {
//            @Override
//            public Iterator<Tuple2<String, Long>> call(Integer integer, Iterator<T> spatiotemporalCircleIterator) throws Exception {
...
//        System.out.println("rawRdd:" + this.rawRDD.getNumPartitions());
